...
namespace ORB_SLAM2 {
    ...
    KeyFrame::KeyFrame(Frame &F, Cache *pCacher) :
            ...
            mvIIKeys(F.mvIIKeys),mvIIKeysUn(F.mvIIKeysUn),mIIDescriptors(F.mIIDescriptors)
            {
        {
          // cout<<"test Keyframe loc 1"<<endl;
        ...
        // cout<<"test Keyframe loc 2"<<endl;


    }

    void KeyFrame::ComputeBoW() {
      // cout<<"test computeBow loc 1"<<endl;
        ...
        if (mIIBowVec.empty() || mIIFeatVec.empty()) {
          // cout<<"test ii descriptor.size="<<mIIDescriptors.size()<<endl;
            vector<cv::Mat> vCurrentDesc = Converter::toDescriptorVector(mIIDescriptors);
            // Feature vector associate features with nodes in the 4th level (from leaves up)
            // We assume the vocabulary tree has 6 levels, change the 4 otherwise
            mpCacher->getMpVocabulary()->transform(vCurrentDesc, mIIBowVec, mIIFeatVec, 4);
            // cout<<"test ii bowvec.size="<<mIIBowVec.size()<<endl;
        }
    }
    ...
    cv::Mat KeyFrame::GetPose() {
        unique_lock<mutex> lock(mMutexPose);
        // cout<<"test KeyFrame::GetPose() "<<endl;
        return Tcw.clone();
    }
