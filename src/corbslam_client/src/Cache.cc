...
namespace ORB_SLAM2 {
    ...
    bool Cache::loadIIVocabulary(const string &strIIVocFile){

      mbifII=true;
      //Load ORB Vocabulary
      cout << endl << "Loading II Vocabulary. This could take a while... "<<strIIVocFile << endl;

      mpIIVocabulary = new ORBVocabulary();
      bool bIIVocLoad = mpIIVocabulary->loadFromTextFile(strIIVocFile);
      if (!bIIVocLoad) {
          cerr << "Wrong path to vocabulary. " << endl;
          cerr << "Falied to open at: " << strIIVocFile << endl;
          exit(-1);
      }
    ...
    void Cache::clearKeyframeDatabase() {
        ...
        // cout<<"test in clearKeyframeDatabase loc 1"<<endl;
        mpKeyFrameDatabase->clear();
        // cout<<"test in clearKeyframeDatabase loc 2"<<endl;
    }
    ...
    void Cache::runUpdateToServer() {
        ...
        while (true) {
            ...
            std::this_thread::sleep_for(std::chrono::microseconds(6000000));
